...
export default function Customization() {
  ...
  async function fetchCustomization() {
    try {
      const res = await axiosInstance.get("/admin/api/all-customizations");
      const data = res.data;
    ...
    <div className="w-full min-h-screen p-8">
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`relative text-sm font-medium pb-2 transition text-gray-400 hover:text-white ${
              activeTab === tab ? "text-white after:absolute after:left-0 after:-bottom-[1px] after:h-[2px] after:w-full after:bg-white" : ""
            }`}
          >
            {tab}
          </button>
      ...
      <div className="mt-8 text-white">
        {activeTab === "Categories" && (
          <div className="space-y-4">
              ...
              categories.map((cat, idx) => (
                <div key={idx}>
                  ...
                  {subCategories?.[cat]?.length > 0 ? (
                    ...
                    <p className="ml-4 text-xs text-gray-500 italic">No subcategories</p>
                  )}
                </div>
              ))
            )}
            ...
            <div className="pt-4 space-x-2">
              ...
              />
              <button onClick={addCategory} className="text-sm bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded-md">
                Add Category
              </button>
            </div>
            ...
            <div className="pt-4 flex items-center gap-2 flex-wrap">
              ...
              <input
                ...
                className="px-3 py-1 rounded-md outline-none text-sm bg-gray-800 text-white border border-gray-600"
              />
              <button onClick={addSubCategory} className="text-sm bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded-md">
                Add Subcategory
              </button>
            </div>
          </div>
        ...
        {activeTab === "Banner" && (
          <div className="space-y-4">
            {banner ? (
              <img src={banner} alt="Platform Banner" className="w-full max-w-[600px] h-auto border border-gray-600 rounded-md" />
            ) : (
