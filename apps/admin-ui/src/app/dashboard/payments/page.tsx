...
async function fetchAdminOrders() {
  const res = await axiosInstance.get("/order/api/admin-orders");
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: "Shop",
        cell: ({ row }: any) => <span className="text-white">{row.original.shop?.name ?? "Unknown Shop"}</span>,
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }: any) => {
          ...
          return <span className="text-green-400 font-semibold">${adminFee.toFixed(2)}</span>;
        },
      },
      {
        ...
        cell: ({ row }: any) => {
          ...
          return <span className="text-white">${sellerEarnings.toFixed(2)}</span>;
        },
      },
      {
        accessorKey: "status",
