...
export default function Users() {
  ...
  const { data: users, isLoading }: UseQueryResult<UsersResponse, Error> = useQuery<UsersResponse, Error, UsersResponse, [string, number]>({
    queryKey: ["all-users", page],
    queryFn: async () => {
      const res = await axiosInstance.get(`/admin/api/all-users?page=${page}&limit=${limit}`);
      return res.data;
    },
  ...
  const filteredUsers = useMemo(() => {
    return allUsers.filter((user) => {
      const matchesRole = roleFilter ? user.role.toLowerCase() === roleFilter.toLowerCase() : true; // WATCH: toLowercase
  ...
  const totalPages = Math.ceil((users?.meta?.totalUsers ?? 0) / limit);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        accessorKey: "role",
        header: "Role",
        cell: ({ row }: any) => <span className="uppercase font-semibold text-blue-400">{row.original.role}</span>,
      },
      {
  ...
  const exportToCSV = () => {
    const csvData = filteredUsers.map((user) => `${user.name},${user.email},${user.role},${user.createdAt}`);
          ...
          <div className="bg-[#1e293b] rounded-2xl shadow-lg w-[90%] max-w-md p-6 relative transition-all duration-200">
            ...
            <div className="mb-6">
              <p className="text-gray-300 leading-6">
                <span className="text-yellow-400 font-semibold">⚠️ Important: </span>
                Are you sure you want to ban <span className="text-red-400 font-medium">{selectedUser.name}</span>? This action can be reverted later.
              </p>
            </div>
