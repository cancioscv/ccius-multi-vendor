...
export default function Loggers() {
  ...
  useEffect(() => {
    const socket = new WebSocket(process.env.NEXT_PUBLIC_LOGGER_WEBSOCKET_URL!);
        ...
        {filteredLogs.length === 0 ? (
          ...
          filteredLogs.map((log, index) => (
            <div key={index} className="whitespace-pre-wrap">
              ...
              <span className="text-purple-400">{log.source}</span> <span className={typeColorMap[log.type]}>[{log.type.toUpperCase()}]</span>{" "}
