      ...
      <div className="my-4">
        <div className="flex items-center justify-between text-xs font-medium text-gray-200 mb-2">
          {orderStatus.map((step, idx) => {
            ...
            const passed = idx <= orderStatus.findIndex((s) => s.toLowerCase() === (order.deliveryStatus || "processing").toLowerCase());
            return (
              <div key={step} className={`flex-1 text-left ${current ? "text-blue-600" : passed ? "text-green-600" : "text-gray-200"}`}>
                {step}
              </div>
            );
          })}
        ...
        <div className="flex items-center">
          {orderStatus.map((step, idx) => {
            const isReached = idx <= orderStatus.findIndex((s) => s.toLowerCase() === (order.deliveryStatus || "processing").toLowerCase());
            return (
