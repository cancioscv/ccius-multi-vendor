...
import { CheckCircle, Globe, PlusCircle, RefreshCcw, Save, Trash2, XCircle } from "lucide-react";
  ...
  return (
    <div className="max-w-2xl space-y-6">
        ...
        <div className="px-4 rounded-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">Connect your own domain to this store.</p>
            </div>
          </div>

          <form onSubmit={handleSubmit((data) => addDomainMutation.mutate(data))} className="mt-4 border-t border-gray-700 pt-4">
            <Input label="Domain Name" placeholder="yourdomain.com" {...register("domainName", { required: "Domain is required" })} />
      ...
      <div className="px-4 rounded-lg">
        ...
        <p className="text-gray-400 text-sm mt-1 mb-3">Manage your custom domain.</p>
        ...
        <div className="mt-4 space-y-4">
          {isLoading ? (
            ...
            domains.map((domain: any) => (
              <div key={domain.id} className="flex justify-between items-center p-3 border border-gray-700 rounded-md">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-sm ${domain.status === "VERIFIED" ? "text-green-400" : "text-yellow-400"}`}>{domain.status}</p>
                  </div>
                </div>

                <div className="flex gap-2">
                  {domain.status !== "Verified" && (
                    <button onClick={() => verifyDomainMutation.mutate(domain.domain)} className="text-blue-400 hover:text-blue-300 transition">
                      <RefreshCcw size={20} />
                    </button>
                  )}
                  <button onClick={() => removeDomainMutation.mutate(domain.id)} className="text-red-400 hover:text-red-300 transition">
                    <Trash2 size={20} />
                  </button>
                </div>
              </div>
        ...
        {verifyDomainMutation.isError && (
          <p className="text-red-500 text-sm mt-3">Error verifying domain! Please check your DNS settings and try again.</p>
        )}
      </div>
      ...
      <div className="px-4 rounded-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">Set up your DNS records for verification.</p>
          </div>
        </div>
        ...
        <div className="mt-4 border-t border-gray-700 pt-4 text-gray-300">
          ...
          <ul className="mt-2 space-y-2 text-sm">
            <li>
              <strong>CNAME:</strong> Set <span className="text-green-400">www</span> to point to{" "}
              <span className="text-blue-400">seller.shondhane.com</span>
            </li>
            <li>
              <strong>A Record:</strong> Point your root domain to <span className="text-blue-400">YOUR_SERVER_IP</span>
            </li>
          </ul>
        </div>
      </div>
