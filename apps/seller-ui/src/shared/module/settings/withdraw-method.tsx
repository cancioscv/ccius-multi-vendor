...
import { Banknote, CheckCircle, Globe, Link } from "lucide-react";
      ...
      <div className="px-4 rounded-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">Manage your Stripe payout settings.</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 rounded-lg">
        {isLoading ? (
          ...
          <div className="p-4 border border-gray-700 rounded-md space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-gray-300 text-sm">
              <div>
                ...
                <p className="text-white">{stripeAccount.business_name || "N/A"}</p>
              </div>
              <div>
                <p className="text-gray-400">Country:</p>
                <p className="text-white">{stripeAccount.country || "Unknown"}</p>
              </div>
              <div>
                <p className="text-gray-400">Payouts Enabled:</p>
                <p className={stripeAccount.payouts_enabled ? "text-green-400" : "text-red-400"}>{stripeAccount.payouts_enabled ? "Yes" : "No"}</p>
              </div>
              <div>
                <p className="text-gray-400">Charges Enabled:</p>
                <p className={stripeAccount.charges_enabled ? "text-green-400" : "text-red-400"}>{stripeAccount.charges_enabled ? "Yes" : "No"}</p>
              </div>
              <div>
                <p className="text-gray-400">Last Payout:</p>
                <p className="text-white">{stripeAccount.last_payout || "No payouts yet"}</p>
              </div>
            </div>
          ...
          <div className="flex flex-col items-center space-y-4 border border-gray-700 p-4 rounded-md">
            ...
            <a href="/product/api/connect-stripe" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md flex items-center gap-2">
