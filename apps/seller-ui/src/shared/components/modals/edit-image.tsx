...
import { Loader, Trash2, UploadCloud, Wand, X } from "lucide-react";
...
async function updateProfilePicture({ editType, imageUrl }: UpdateProfilePictureProps) {
  if (!editType || !imageUrl) throw new Error("Missing required fields!");

  const response = await axiosInstance.put("/seller/api/update-profile-picture", {
    editType,
    imageUrl,
  });
...
export default function EditImage({ editType, onClose }: EditImageProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null); // Watch: I think this is state is not necessary
  ...
  async function uploadToImageKit(file: File) {
    ...
    try {
      ...
      const response = await axiosInstance.post("/seller/api/upload-image", payload);
    ...
    <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-60 z-50">
      <div className="bg-gray-800 p-6 rounded-lg w-[450px] text-white">
        ...
        <div className="flex justify-between items-center pb-3 mb-4">
          <h2 className="text-lg font-semibold">{editType === "cover" ? "Edit Cover Photo" : "Edit Profile Picture"}</h2>
            ...
            <div className="grid grid-cols-2 gap-3 max-h-[250px] overflow-y-auto">
              {enhancements.map(({ label, effect }) => (
                <button
                  ...
                  disabled={processing}
                >
                  {processing && activeEffect === effect ? <Loader size={18} className="animate-spin" /> : <Wand size={18} />}
                  {label}
                </button>
              ))}
            </div>
