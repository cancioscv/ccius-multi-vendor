...
export default function ChatPage() {
  ...
  const { data: messages = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await axiosInstance.get(`/chat/api/seller-messages/${conversationId}?page=1`);
  ...
  const { data: conversations, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosInstance.get("/chat/api/seller-conversations");
  ...
  function handleChatSelect(chat: any) {
    ...
    <div className="w-full">
      <div className="flex h-screen shadow-inner overflow-hidden bg-gray-950 text-gray-200">
        ...
        <div className="w-[320px] border-r border-gray-800 bg-gray-950">
          <div className="p-4 border-b border-gray-800 text-lg font-semibold">Messages</div>
          <div className="divide-y divide-gray-900">
            {isLoading ? (
              <div className="text-center py-5 text-sm">Loading...</div>
            ) : chats.length === 0 ? (
              <p className="text-center py-5 text-sm">No conversation available yet!</p>
            ) : (
              chats.map((chat) => {
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-semibold text-white">{chat.user?.name}</span>
                          {chat.user?.isOnline && <span className="w-2 h-2 rounded-full bg-green-500" />}
                        </div>
                        <div className="flex items-center justify-between">
                          <p className="text-xs text-gray-400 truncate max-w-[170px]">{chat.lastMessage || ""} </p>
                          {chat?.unreadCount > 0 && (
                            <span className="ml-2 text-[10px] bg-blue-600 text-white rounded-full px-1.5 py-0.5">{chat?.unreadCount}</span>
                          )}
                        </div>
                      </div>
                    </div>
        ...
        <div className="flex flex-col flex-1 bg-gray-950">
          {selectedChat ? (
            <>
              ...
              {/* Messages */}
              <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4 text-sm" ref={messageContainerRef}>
                {messages.map((msg: any, index: number) => (
                  <div key={index} className={`flex flex-col ${msg.senderType === "seller" ? "items-end ml-auto" : "items-start"} max-w-[80%]`}>
                    <div
                      className={`${
                        msg.senderType === "seller" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-200"
                    ...
                    <div
                      className={`text-[11px] text-gray-400 mt-1 flex items-center gap-1 ${
                        msg.senderType === "seller" ? "mr-1 justify-end" : "ml-1"
                      }`}
                    >
