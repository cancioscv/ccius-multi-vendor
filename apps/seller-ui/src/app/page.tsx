...
async function fetchProducts() {
  ...
  const products = res.data.products?.filter((product: any) => !product.startingDate);
  return products;
}
      ...
      {isLoading ? (
        ...
        <div className="w-full bg-gray-900 min-h-screen">
          ...
          <div className="w-full px-3 pt-2">
            <button onClick={() => router.push("/dashboard")} className="flex items-center gap-2 text-gray-300 hover:text-white transition mb-4">
              <ArrowLeft size={20} />
          ...
          <div className="relative w-full flex justify-center bg-gray-800">
            <Image
              src={seller?.shop?.coverBanner || "/placeholder.png"}
              alt="Seller Cover"
          ...
          <div className="w-[85%] lg:w-[70%] mt-[-50px] mx-auto relative z-20 flex flex-col lg:flex-row gap-6">
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg flex-1">
              <div className="flex flex-col md:flex-row gap-6 items-center md:items-start">
                <div className="relative w-[100px] h-[100px] rounded-full border-4 border-gray-700 overflow-hidden">
                  <Image src={seller?.shop?.avatar || "/placeholder.png"} alt="Seller Avatar" layout="fill" objectFit="cover" />
                  {seller?.id && (
                    <label className="absolute bottom-1 right-1 bg-gray-700 p-2 rounded-full cursor-pointer" onClick={() => setEditType("avatar")}>
                ...
                <div className="flex-1 w-full">
                  <h1 className="text-2xl font-semibold text-white">{seller?.shop?.name}</h1>
                  ...
                  <div className="flex items-center gap-3 mt-3 text-gray-400">
                    <Clock size={18} />
                    <span>{seller?.shop?.openingHours || "Mon - Sat: 9 AM - 6 PM"}</span>
                  </div>

                  <div className="flex items-center gap-2 mt-3 text-gray-400">
                    <MapPin size={18} /> <span>{seller?.shop?.address || "No address provided"}</span>
                  </div>
                </div>
            ...
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-full lg:w-[30%]">
              ...
              <div className="flex items-center gap-3 mt-3 text-gray-400">
                <Calendar size={18} />
                <span>Joined At: {new Date(seller?.shop?.createdAt).toLocaleDateString()}</span>
              </div>

              {seller?.shop?.website && (
                <div className="flex items-center gap-3 mt-3 text-gray-400">
                  <Globe size={18} />
                  <Link href={seller?.shop?.website} className="hover:underline text-blue-400">
                    {seller?.shop?.website}
                  </Link>
                </div>
              )}
              ...
              {seller?.shop?.socialLinks && seller?.shop?.socialLinks.length > 0 && (
                <div className="mt-3">
                  <h3 className="text-white text-lg font-medium">Follow Us:</h3>
          ...
          {editType && <EditImage editType={editType} onClose={() => setEditType(null)} />}
        </div>
      )}
    </>
  );
